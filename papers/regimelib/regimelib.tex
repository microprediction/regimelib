...
\begin{abstract}
A regime is a finite-state Markov chain the market does not observe, and a regime-switching model lets any
parameter of a standard model take a different value in each state. For the affine and exponential-L\'evy models
in common use the pricing problem reduces to a linear system of ordinary differential equations,
$a' = (Q + \diag g(t))\,a$, in which the generator $Q$ of the chain and a model-specific forcing $g$ are the only
ingredients. We expand the solution in the mean holding time $\eps$ of the chain to any order, with an initial layer
that carries the starting regime, and identify the first correction as a Green--Kubo matrix built from the group
inverse of $Q$ and the memory term as the group inverse applied to the forcing. The series is asymptotic rather
than convergent, and it diverges at high Fourier frequency where the forcing is large; a hybrid engine that
solves the reduced system numerically at those frequencies restores the price. Options on bonds and swaps are
priced by Gil--Pelaez integrals conditioned on the regime at expiry, which turns Jamshidian's decomposition into one
crossing per regime; equity options under stochastic rates use a discounted characteristic function so that the
discount and the return share the regime path. Sensitivities to per-regime parameters and to the switching rates
themselves follow in closed form from the derivative of the group inverse. The methods are implemented in
\rl{}, a Python library that mirrors \ql{}'s classes and adds a chain in front of the parameters; every model has a
frozen limit in which it is \ql{}'s model, and a suite of certificates checks that limit against \ql{}'s engines
and the switching case against an independent referee.
\end{abstract}
...
This paper does three things. First it states the reduced system for the models in \ql{} \citep{quantlib} in
one form, the forcing $g_i(t)$ of regime $i$, and derives the expansion of its solution to any order with the
initial layer, identifying the terms: the averaged model at order zero, a Green--Kubo matrix of the forcing's
fluctuations at order one, and a memory of the starting regime through the group inverse of the generator. The
series is asymptotic; we say where it fails and what to do there. Second it prices the instruments that a library
must price---bonds, European and digital options, options on bonds and swaps, caps, Asian, barrier, American and
Bermudan instruments, credit default swaps, equity options under stochastic rates---in terms of the reduced
system, with two constructions that appear to be new: Gil--Pelaez integrals conditioned on the regime at expiry,
which make Jamshidian's decomposition \citep{jamshidian1989} work regime by regime, and a discounted characteristic
function for hybrid models on a common chain. Third it describes \rl{}, a Python implementation in \ql{}'s mold:
the same class names and parameters with a chain in front, and a suite of certificates in which every model is
checked against \ql{} in the limit where all regimes coincide and against a numerical, grid or Monte Carlo
referee with switching on. The certificates are the paper's evidence; the tables in
Section~\ref{sec:numerics} are generated by a script in the repository.
...
the point here is that one form covers the whole catalogue, including the terminal exponents that bond options
need and the complex forcings of characteristic functions, and that the models outside the catalogue are exactly
those with a non-commuting switched operator. Those are treated in Section~\ref{sec:firstorder}.
...
time $\tau/\eps$. The construction is standard singular perturbation \citep{yin2013}; what matters for pricing is
the form of the terms.
...
when the switched operators do not commute (Section~\ref{sec:firstorder}). The memory term $m_i$ is the group
inverse applied to the centred forcing, evaluated in the starting regime; it is what distinguishes the price
seen from regime $i$ from the price seen from regime $j$, and it decays with the layer as the chain forgets its
start. Higher orders follow by the same recursion and the library computes them to any order, with the
integrals done exactly when $g$ is an exponential sum and by Chebyshev series otherwise. The certificate
\texttt{verify\_theorem.py} beside this paper evaluates the residual of the order-one approximation, layer included,
...
stop decreasing beyond some order, and at fixed order they diverge beyond some frequency. Two consequences are
built into the library: the engine reports the size of the last term kept and stops at the best truncation when
asked, and at Fourier nodes where the last term is not smaller than the one before it the reduced system is
solved numerically instead of expanded, so that the price is a hybrid of the expansion where it converges and
the numerical solution where it does not. The geometric Asian option, whose characteristic function decays
slowly in $u$, is where the hybrid first became necessary; replacing the diverging nodes by the averaged value,
the earlier remedy, left an error of $10^{-3}$ that the hybrid removed.
\end{remark}
...
probability of the form $\E[e^{-\int_0^T r}\,e^{-c x_T}\,\mathbf 1\{x_T < x^*_j\}\,\mathbf 1\{y_T = j\}]$, and
Gil--Pelaez inversion gives it from $\Psi_j(c - iu)$, the reduced system with the terminal exponent $c - iu$ and
$a_0 = e_j$:
\begin{equation}
...
Black--Scholes--Hull--White and Heston--Hull--White engines to $10^{-8}$ and $10^{-6}$.
...
the expansion of Section~\ref{sec:expansion} still applies to the operator-valued system, with the first
correction now
\begin{equation}
...
(Section~\ref{sec:software}), $\partial P/\partial\theta_i$, $\partial P/\partial\sigma_i$ and $\partial P/\partial Q_{ab}$
...
priced by \rl{} and by \ql{}'s engine for the same instrument. Table~\ref{tab:orders} is the expansion with
switching on: a payer swaption under a two-regime Vasicek model (mean levels 6\% and 2\%, volatilities 1.5\% and
0.8\%) at each order against the numerical solution of the reduced system, for four holding times. The error
falls with the order until the asymptotic nature of the series shows, later for faster chains, and the order at
which it stops improving is what the adaptive engine selects. Table~\ref{tab:timings} gives timings on a laptop;
they say where the expansion earns its keep and where it does not. For a bond, one evaluation of the series, it
is the fastest route. For a Fourier integral the numerical engine solves the reduced system for every node in a
single batched call, and the series evaluated node by node at order four costs more; the expansion's value there
is the formula, the diagnostics and the greeks under the integral, not the wall clock. The Monte Carlo referee
is pure Python and priced per path; it is a referee, not an engine. Table~\ref{tab:greeks} gives the parameter
sensitivities. All four tables are written by
\texttt{papers/regimelib/verify\_tables.py} in the repository.

\begin{table}[t]\centering\footnotesize
\resizebox{\textwidth}{!}{\input{tables_frozen}}
...
\begin{table}[t]\centering\small
\input{tables_greeks}
\caption{Parameter sensitivities of the first-order Vasicek bond under a three-regime chain, from the formula of
Section~\ref{sec:greeks} and from central differences of the engine at order one.}
\label{tab:greeks}
\end{table}
...
diagnostics. There is no third category.
...
The referee found the two errors worth recording. The Asian forcing was first written in calendar time where the
reduced system runs in time to maturity---invisible in the frozen limit, which is symmetric, and a 7\% error
with switching on. And the cross term of the first-order coupon-bond formula needed the symmetric part of $K$
where the antisymmetric part had been used, a $10^{-4}$ discrepancy against the referee that a reversible test
chain would never have shown.

Closed forms live in \texttt{regimelib.symbolic}: the two-state Vasicek bond to second order, the first-order
bond under any chain for Vasicek, CIR (integrals closed by the Riccati identity $\int B^2 = \frac2{\sigma^2}(T - kI_1 - B(T))$)
and Vasicek with jumps, the exact two-state constant-forcing solution, and the parameter greeks of
Section~\ref{sec:greeks}, each checked against the engine at the same order to round-off.

\section{Conclusion}

The reduced system \eqref{eq:reduced} is a small object---a generator and a forcing---and it carries the whole
catalogue of affine and exponential-L\'evy models under a hidden regime. Its fast-switching expansion has terms
one can read: the averaged model, the integrated autocovariance of the parameters, the memory of the start. It is
asymptotic, and a pricing engine built on it must say so and must know what to do where it fails; the hybrid with
the numerical solution does. The instruments a library needs follow from the same system by conditioning on the
regime at expiry, which is what makes Jamshidian's trick and Gil--Pelaez's inversion work regime by regime, and by
a discounted characteristic function when the rate and the underlying share the chain. What remains is what the
certificates do not yet cover: the second order of the first-order tier, and ports of the reference
implementation to other languages under the same parity contract.
