...
\usepackage{amsmath,amssymb,amsthm,booktabs,graphicx,hyperref,url,microtype}
...
\begin{align}
  ...
  a^{(1)}_i(\tau) &= a^{(0)}(\tau)\Big(\int_0^\tau K(s)\dd s \;-\; m_i(\tau)\Big), \qquad
  K(s) = -\,\pi\cdot\big(\tilde g(s)\odot Q_1^{\#}\tilde g(s)\big), \quad m_i(\tau) = \big(Q_1^{\#}\tilde g(\tau)\big)_i, \label{eq:order1}
\end{align}
...
integrals done exactly when $g$ is an exponential sum and by Chebyshev series otherwise.
...
and Table~\ref{tab:greeks} the parameter sensitivities. All four tables are written by
