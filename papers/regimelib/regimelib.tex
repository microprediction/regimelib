...
The Reduced System and Its Asymptotics}
...
the Gil--Pelaez integrals are conditioned on the regime at expiry. Every numerical claim is checked against \ql{} in the limit where the regimes coincide and against an
independent referee with switching on, and the implementation is public.
\end{abstract}

\section{Introduction}

Hamilton's regime-switching model \citep{hamilton1989} put a hidden finite-state Markov chain behind the
parameters of a time series, and \citet{naik1993} priced options with the volatility driven by such a chain. The
pricing problem under a regime-switching model is a coupled system, one equation per regime, and for the affine
and exponential-L\'evy models in use the state dependence factors out, leaving a linear system of ordinary
differential equations in which the chain and the model meet only through the generator and a forcing term. The
reduction, and the asymptotic expansion of its solution when the chain is fast in the mean holding time, were
carried out for the Ornstein--Uhlenbeck bond and the survival probability in the author's thesis
\citep{cotton2001}. The two-state reduction for the Vasicek bond appears again in \citet{elliott2002}, and
American options with switching in \citet{buffington2002}.

\citet{yin2009option} expanded the coupled
Black--Scholes equations in the holding time of a fast chain, with the averaged volatility at leading order and
uniform error bounds, and \citet{basu2009} corrected the risk-minimising option price for a fast and for a slow
chain. The general theory of two-time-scale Markov chains, forward and backward equations without a potential
term, is \citet{yin2013} and \citet{yin2009backward}; \citet{nguyen2011,nguyen2013} carried the same expansion,
with a terminal layer and uniform error bounds, to the backward equation of a switching diffusion, where the
regime enters through the diffusion operator rather than a potential. The parallel expansion for fast stochastic
volatility, with a chain among the admissible drivers, is \citet{sircar1999}, and for a fast diffusion driving the
volatility \citet{khasminskii2005}.
...
The implementation, \rl{}, follows \ql{}'s classes with a chain in front of the parameters. Every model reduces
to \ql{}'s when the regimes coincide, and a suite of certificates checks that limit against \ql{} and the
switching case against a numerical, grid or Monte Carlo referee. The tables below are written by a script in the
repository.
...
The two-state case of the reduction, a pair of linear ODEs, is in \citet{cotton2001} and again in \citet{elliott2002} for the Vasicek bond.
...
time $\tau/\eps$. The construction is standard singular perturbation \citep{yin2013,nguyen2013}; the potential term $\diag g$ is what
separates the reduced system from the backward equations treated there. The terms have a form that can be stated.
...
Table~\ref{tab:greeks} gives the parameter sensitivities.
...
The implementation is released under the MIT licence at \url{https://github.com/microprediction/regimelib}, on
PyPI as \texttt{regimelib}, with documentation at \url{https://regimeswitching.org/python/}. A model in the
all-orders tier is a statement of its forcing; a model whose reduction is not exact goes to the first-order tier
with its diagnostics. Instruments accept \ql{}'s payoff, exercise and date objects, and the greeks are computed by
differentiating the pricing formula.

Every certificate checks the frozen limit against \ql{} and the switching case against a referee that does not
share the method: the numerical solution of \eqref{eq:reduced} for the expansion, the coupled PDE on a grid for
the first-order tier, Monte Carlo with exact regime paths for the grids. The frozen limit alone is not enough,
and two blind spots fix the design. With all regimes equal, a forcing integrates to the same value in calendar
time and in time to maturity, so the frozen limit cannot detect the direction of time in a time-dependent
forcing; with switching on, the Asian option moves by 7\% between the two. And a reversible chain has a symmetric
Green--Kubo matrix, so only a non-reversible test chain separates the symmetric part of $K$ from the whole in a
cross term, a $10^{-4}$ effect in the first-order coupon-bond formula. The certificates therefore run with
switching on and on non-reversible chains.
...
checked against the engine at the same order to round-off. The tables are written by
\texttt{papers/regimelib/verify\_tables.py} and the theorem's certificate is \texttt{verify\_theorem.py} beside it.
